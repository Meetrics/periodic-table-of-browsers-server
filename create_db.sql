...
CREATE TABLE IF NOT EXISTS `browsers`.`Browsers` (
  ...
  UNIQUE INDEX `UNIQUE` (`browser` ASC, `version` ASC, `mobile` ASC, `os` ASC, `userAgent` ASC),
...
CREATE TABLE IF NOT EXISTS `browsers`.`Browsers_has_WindowProperties` (
  `Browsers_userAgent` VARCHAR(64) NOT NULL,
  `WindowProperties_id` INT(11) NOT NULL,
  PRIMARY KEY (`Browsers_userAgent`, `WindowProperties_id`),
  INDEX `fk_Browsers_has_WindowProperties_WindowProperties1_idx` (`WindowProperties_id` ASC),
  INDEX `fk_Browsers_has_WindowProperties_Browsers_idx` (`Browsers_userAgent` ASC),
  CONSTRAINT `fk_Browsers_has_WindowProperties_Browsers`
    FOREIGN KEY (`Browsers_userAgent`)
    REFERENCES `browsers`.`Browsers` (`userAgent`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `browsers`.`Browsers_has_DocumentProperties` (
  ...
  PRIMARY KEY (`Browsers_userAgent`, `DocumentProperties_id`),
  INDEX `fk_Browsers_has_DocumentProperties_DocumentProperties1_idx` (`DocumentProperties_id` ASC),
  INDEX `fk_Browsers_has_DocumentProperties_Browsers1_idx` (`Browsers_userAgent` ASC),
